...
- [x] **Single-choice picker replacement behavior** — In app pickers where all options are visible and the user may select only one—especially two-option choices—selecting a different option replaces the current selection immediately. The shared attribute, feat, and spell selection logic handles one-choice replacement without requiring the first option to be deselected.
...
- [ ] **Wound-state pacing** — Recheck whether the current **Downed** to **Stunned** progression slows lethality too much. Explore reversing the order so a fighter becomes **Stunned** first while still standing but unable to use skills, with **Downed** as the later injury state; also compare **Stunned** as a parallel condition rather than an extra injury step. Preserve "miraculous return" moments for **Downed** fighters and check how either approach affects lethality, recovery, targeting, Mercy Kill, companion behavior, and state-specific effects.
