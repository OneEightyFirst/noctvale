...
Removed the Universal **Dodge** feat so the maneuver name is clear. **Shoulder to Shoulder** now grants **+1 Might defense die** instead of **+1 Skill defense die**. **Entangle** now explicitly prevents defensive maneuvers while **Movement** is 0. **Cleave** checks for its follow-up target after defensive maneuver movement. **Constables** was removed from the Mortal Tradition list; a todo now tracks designing a replacement Mortal Tradition.

**Reasoning:** The old 2:1 cross-block kept mismatched defense usable but did not make **Might** and **Skill** feel meaningfully different at the table. Defensive maneuvers preserve the same two-success cost while making the mismatch create position: **Skill** escapes brute force by giving ground, and **Might** breaks finesse by driving the attacker back. Keeping movement after Wounds preserves Noctvale's lethal combat rhythm, while limiting **Shove** to melee and making **Dodge** available against ranged attacks avoids giving ranged defense an awkward Might-based push with no clear physical source. Stopping **Shove** at terrain or fighters keeps the push physically readable, and the ledge check gives strong fighters a dangerous positioning payoff without making every edge contact an automatic fall.

## 2026-07-13 — Closed cross-domain feat loopholes; capped beast marks at 2; defined Ranged Attack Action

**Decision:** Reviewed all Archetype, Domain, and Universal feats for exploitable combos, including the case where a fighter buys a feat **outside their own Archetype or Domain** through post-game Advancement. Three fixes:

1. Defined **Ranged Attack Action** — resolving one weapon's **Primer Roll** (if any) and **Attack Sequence** during a **Ranged** action. A **Ranged** action normally resolves 1 Ranged Attack Action; **Gunslinger** resolves 2 (one per Pistol). **Aim**, **Deadeye**, and **Patient Shot** now key their bonus to "next **Ranged Attack Action**" instead of the ambiguous "next Ranged action" / "next Ranged attack" — so a **Gunslinger** shot only gets the Aim bonus on the first Pistol resolved, not both.
2. Added an explicit **Fighter must have the Mortal keyword** prerequisite to **Firearms** and **Gunslinger**, which previously relied only on being listed under the Mortal domain — exactly the gate that an outside-Archetype/Domain Advancement purchase bypasses. Without it, a non-Mortal retinue could unlock firearms purchasing for the whole roster from a single fighter's Advancement pick.
3. **Wild Aspect** now grants the **Werebeast** keyword itself (in addition to the beast mark) instead of requiring the fighter already have it. This makes the feat self-sufficient rather than dependent on the **Werebeasts** tradition, and turns the outside-Archetype/Domain purchase path into an intentional feature: a **Mortal** (or any other domain) retinue can spend Advancement to field one fighter with a beast mark, at the cost of that fighter also picking up the **Werebeast** keyword's downsides (silver weapon vulnerability, etc.).
4. Capped **beast marks** at **2 per fighter** in both `rules/traditions.md` and `rules/feats.md#wild-aspect`. **Wild Aspect** has no explicit repeat limit in its own text, and stacking all 4 marks (Wolf/Rat/Bear/Serpent) on one fighter was clearly stronger than any mark was individually costed for.

**Reasoning:** Advancement's "Feat (outside Archetype or Domain)" option (15 XP) only re-checks "any other prerequisite the feat itself lists" — it does not re-check domain/archetype membership, since bypassing that is the point of the option. **Firearms**/**Gunslinger** needed a printed keyword gate because their effect is a blanket retinue-wide unlock with no natural downside; **Wild Aspect** did not, because turning any fighter into a beast-kin creature is exactly the kind of one-off flavor pickup Advancement is meant to enable, and the feat itself now carries the full package (mark, keyword, and the vulnerabilities that come with it). The beast-mark cap is a separate balance fix — repeatable feat picks currently had no stated ceiling, so an experienced fighter could accumulate every mark in the game.

Other combos flagged in review (Iron Discipline + Last Gasp tankiness, Torpor's uncapped charge-up, Second Sight + Pathfinder survival-roll insurance, Convoke's flat +1 vs. Blood for the Rite's new die mechanic) were left as-is — not clearly broken, and bounded by once-per-battle/round limits or campaign pacing.

## 2026-07-13 — Blood for the Rite grants an extra die instead of a flat +1

**Decision:** **Blood for the Rite** now adds **1d6** to the triggered attribute check or **casting roll**, then discards the lowest single die, instead of adding a flat **+1**.

**Reasoning:** A flat +1 is a weak, swingy payoff for spending a friendly fighter's **Wound**. Rolling an extra die and keeping the best result scales the bonus with the roll itself (bigger on high rolls, still helps on low ones) and gives the cost a payoff that feels worth the blood spent.

## 2026-07-13 — Replaced 2d6 advancement tables with a direct-purchase XP cost ladder; added mid-campaign Caster

**Decision:** Scrapped the 2d6 roll-based **Feat Advancement Table** (previously a placeholder) and **Attribute Advancement Table**. Advancement is now a flat, direct-purchase list — no rolling. Spend XP to buy exactly what you want, in ascending cost order: **Feat** (3 XP) → **Wi**/**Sa** +1 (6) → **Mt**/**Sk** +1 (7) → **W** +1 (8) → **Caster** (9) → **M** +1 (10) → **Df** +1 (11) → **CC**/**RC** +1 (12) → **Feat outside Archetype/Domain** (15). The **Vampire** tax stays **+2 XP** on any tier.

The top tier lets a fighter buy into an **Archetype** or **Domain** feat list their own keywords don't grant (still subject to any other feat prerequisite, such as **Caster**), without granting the matching keyword itself. Excludes **Proficiency** feats — weapon access stays bound to a fighter's own **Archetype**. Priced above **CC**/**RC** since it bends the normal keyword-gating rule rather than just raising a number.

Added a stacking guard: raising the same attribute more than **+1** above its static **ancestry baseline** (the Baseline Ancestry Profiles value — not the value after recruitment bonuses, feats, or prior advancement) requires reducing a different attribute by **1** (minimum **1**) for each point beyond that first **+1**. This resolves the "Attribute boost tradeoffs" idea — the tradeoff applies to post-game advancement (not recruitment boosts directly), triggers per point beyond +1 above the fixed ancestry value, and any attribute can be the one reduced. Measuring against the static ancestry baseline rather than each fighter's recruitment-recorded value keeps the trigger point a fixed, always-known number instead of a build-dependent one — simpler to track at the table, at the cost that a fighter who already spent their recruitment bonus on **CC**, **RC**, or another purchasable attribute reaches the tradeoff on their very first advancement purchase of that same attribute.

Added **Caster** as its own advancement tier, closing the "Mid-campaign Caster" todo. Requires **Wi 4+** or **Sa 4+** (whichever governs the fighter's Domain casting roll); grants the **Cast** action and 1 Domain spell. The existing recruitment-time restrictions (1 Caster per retinue, forbidden alongside **Mortal**) still apply.

**Reasoning:** A pure 2d6 roll table for Feats couldn't guarantee a fighter got a feat that matched their Archetype/Domain/proficiency build — a random result could be useless or illegal for that fighter. Direct purchase from the same eligible-feat lists used at recruitment avoids that mismatch entirely and needed no new list to maintain.

For Attributes, sized the cost ladder against the earning rules in this file: a typical fighter nets roughly 1.3–1.5 XP/game (survival +1 dominates; kill and Leader-kill XP is concentrated on a few fighters per battle; scenario and post-game bonuses are minor by comparison), or **~13–15 XP over a 10-game campaign** for a typical fighter and **~20–25 XP** for a retinue's most XP-favored fighter. Pricing **CC**/**RC** at 12 XP means even a favored fighter spends most of a full campaign's bank on one purchase, and stacking a second point on the same top-tier stat (~24 XP, plus the forced tradeoff elsewhere) is realistically out of reach inside 10 games without abandoning every other advancement — keeping **CC**/**RC**/**Df** rare, late-campaign, and a real specialization choice rather than a default buy. Ordering the rest of the ladder (**Wi**/**Sa** cheapest attribute tier, **M** pricier than **Caster**, **Df** just under **CC**/**RC**) reflects each attribute's relative combat weight.
